...
class RL_Trainer:
    def __init__(
        ...
        self.vec_env = make_vec_env(
            TactileEnv, n_envs=self.num_envs, env_kwargs=self.env_kwargs
        )
        self.eval_env = make_vec_env(
            TactileEvalEnv, n_envs=self.num_eval_envs, env_kwargs=self.env_kwargs
        )
    ...
    def train(self):
        ...
        if not self.resume:
            self.model = self.algo(
                ...
                self.vec_env,
                verbose=1,
                tensorboard_log=self.checkpoint_path,
            )
        else:
            self.load_model()
        self.eval_callback = EvalCallback(
            ...
            eval_freq=self.task.total_steps * 10,
        )
        self.model.learn(
            total_timesteps=self.training_iterations * self.task.total_steps,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_train_envs", type=int, default=4, help="num of envs to train policy"
    )
    parser.add_argument(
        "--num_eval_envs", type=int, default=1, help="num of envs to eval policy"
    )
    parser.add_argument(
        "--max_iteration", type=int, default=200, help="max iterations for training"
    )
    parser.add_argument(
        "--resume", action="store_true", help="whether or not resume best_model"
    )
    parser.add_argument(
        "--off_screen", action="store_true", help="whether or not use off screen mode"
    )
    parser.add_argument("--task", default="box_open", help="task to train")
    parser.add_argument(
        "--algorithm", default="PPO", help="algorithm for training, SAC or PPO"
    )  # 1 means PPO, 2 means SAC
    parser.add_argument(
        "--obs",
        default="state",
        help="whether or not to use tactile in observations, state or tactile",
    )
    parser.add_argument(
        "--loss",
        default="state",
        help="whether or not to use tactile in loss, state or tactile",
    )
