"""
a class to describe soft elastic objects with mpm
"""
...
class MPMObj:
    def __init__(self, dt=5e-5, sub_steps=80, obj_name=None, space_scale = 1.0, obj_scale = 1.0, density = 2, rho = 1):
        ...
        self.gravity = ti.Vector([0.0, 0.0, -9.80665]) # m/s^2
    ...
    def p2g(self, f:ti.i32):
        for p in range(self.n_particles): # Particle state update and scatter to grid (P2G)
            ...
            cauchy = 2 * self.mu_0[None] * (self.F_new[f, p] - r) @ self.F_new[f, p].transpose() + ti.Matrix.identity(float, 3) * self.lambda_0[None] * J * (J - 1)
