...
class MultiObj:
    def __init__(
        ...
        self.border_line = (
            np.array(
                [
                    2 * height_limit * (30 / 43) - height_limit,
                    2 * height_limit * (33 / 43) - height_limit,
                ]
            )
            * self.obj_scale
        )
        ...
        self.COM_t1 = ti.Vector.field(
            3, dtype=float, shape=(self.sub_steps), needs_grad=True
        )
        self.H = ti.Matrix.field(
            3, 3, dtype=float, shape=(self.sub_steps), needs_grad=True
        )
        self.R = ti.Matrix.field(
            3, 3, dtype=float, shape=(self.sub_steps), needs_grad=True
        )
        self.U = ti.Matrix.field(
            3, 3, dtype=float, shape=(self.sub_steps), needs_grad=True
        )
        self.S = ti.Matrix.field(
            3, 3, dtype=float, shape=(self.sub_steps), needs_grad=True
        )
    ...
    def H_svd_grad(self, f: ti.i32):
        ...
        u_term = (
            self.U[f]
            @ (
                (ff * (ut @ self.U.grad[f] - self.U.grad[f].transpose() @ self.U[f]))
                @ self.S[f]
            )
            @ vt
        )
        v_term = self.U[f] @ (
            self.S[f]
            @ (
                (ff * (vt @ self.V.grad[f] - self.V.grad[f].transpose() @ self.V[f]))
                @ vt
            )
        )
        return u_term + v_term + s_term
    ...
    def compute_COM(self, f: ti.i32):
        for p in range(self.n_particles):
            if self.titles[p][0] == 2:
                ...
                self.COM_t1[f] += (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p]
                ) / ti.cast(self.n_particles, ti.f32)

    @ti.kernel
    def compute_H(self, f: ti.i32):
        for p in range(self.n_particles):
            if self.titles[p][0] == 2:
                self.H[f][0, 0] += (self.x_0[f, p] - self.COM_t0[f])[0] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[0]
                self.H[f][0, 1] += (self.x_0[f, p] - self.COM_t0[f])[0] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[1]
                self.H[f][0, 2] += (self.x_0[f, p] - self.COM_t0[f])[0] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[2]
                self.H[f][1, 0] += (self.x_0[f, p] - self.COM_t0[f])[1] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[0]
                self.H[f][1, 1] += (self.x_0[f, p] - self.COM_t0[f])[1] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[1]
                self.H[f][1, 2] += (self.x_0[f, p] - self.COM_t0[f])[1] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[2]
                self.H[f][2, 0] += (self.x_0[f, p] - self.COM_t0[f])[2] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[0]
                self.H[f][2, 1] += (self.x_0[f, p] - self.COM_t0[f])[2] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[1]
                self.H[f][2, 2] += (self.x_0[f, p] - self.COM_t0[f])[2] * (
                    self.x_0[f, p] + self.dt * self.v_0[f + 1, p] - self.COM_t1[f]
                )[2]
