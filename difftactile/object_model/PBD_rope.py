...
class PBDRope:
    def __init__(
        ...
        if rest_length is None:
            ...
            self.bending_length = (
                4 * self.p_rad
            )  # the distance between two sets of segments
            ...
            self.length = (
                self.rest_length * (self.n_particles - 1) + self.p_rad * 2
        ...
        self.init_pos = ti.Vector.field(
            3, dtype=ti.f32, shape=(self.n_particles)
        )  # init_position
        self.target_pos = ti.Vector.field(
            3, dtype=ti.f32, shape=(self.n_particles)
        )  # target_position
        self.tmp_pos = ti.Vector.field(
            3, dtype=ti.f32, shape=(self.sub_steps, self.n_particles), needs_grad=True
        )
        self.tmp_vel = ti.Vector.field(
            3, dtype=ti.f32, shape=(self.sub_steps, self.n_particles), needs_grad=True
        )
        self.dpos = ti.Vector.field(
            3, dtype=ti.f32, shape=(self.sub_steps, self.n_particles), needs_grad=True
        )
        ...
        self.travel_info = ti.Vector.field(
            1, dtype=ti.f32, shape=(self.sub_steps, self.n_particles), needs_grad=True
        )
    ...
    def init(self, pos: ti.types.vector(3, ti.f32), ori: ti.types.vector(3, ti.f32)):
        ...
        for i in range(1, self.n_particles):
            self.init_pos[i] = (
                self.init_pos[0] + self.init_ori[None] * self.rest_length * i
            )
    ...
    def set_target(self, pos: ti.types.vector(3, ti.f32)):
        ...
        for i in range(1, self.n_particles):
            self.target_pos[i] = (
                self.target_pos[0] + self.target_ori[None] * self.rest_length * i
            )
    ...
    def load_step_from_cache(
        ...
        cache_travel_info: ti.types.ndarray(),
    ):
        for p in range(self.n_particles):
    ...
    def memory_to_cache(self, t):
        ...
        self.cache[cur_step_name]["travel_info"] = torch.zeros(
            (self.n_particles, 1), dtype=TC_TYPE, device=device
        )
        self.add_step_to_cache(
            0,
            self.cache[cur_step_name]["pos"],
            self.cache[cur_step_name]["vel"],
            self.cache[cur_step_name]["travel_info"],
        )
