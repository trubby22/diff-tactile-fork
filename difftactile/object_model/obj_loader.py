...
class ObjLoader:
    def __init__(self, data_path, particle_density=32):
        ...
        self.particle_density = (
            particle_density  # # of particles in one dimension of cube
        )
    ...
    def generate_particles(self):
        self.raw_mesh = trimesh.load(self.data_path, force="mesh", skip_texture=True)
        self.normalized_mesh = self.cleanup_mesh(self.normalize_mesh(self.raw_mesh))
        self.voxelized_mesh = self.normalized_mesh.voxelized(
            pitch=1.0 / self.voxel_resolution
        ).fill()
    ...
    def sample_cube(self):
        ...
        if use_non_uniform_mesh_distribution:
            ...
            geometric_distribution = np.geomspace(
                original_start, original_end, num_samples, dtype=np.float32
            )
            ...
            # Clamp the values to ensure they stay within [-0.5, 0.5]
            x = np.clip(x, -0.5, 0.5)

            y = np.linspace(-0.5, 0.5, self.particle_density + 1, dtype=np.float32)
            assert x.shape == y.shape
            particles = np.stack(np.meshgrid(x, y, y, indexing="ij"), -1).reshape(
                (-1, 3)
            )
        else:
