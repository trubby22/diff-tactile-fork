...
class RigidObj:
    def __init__(
        ...
        self.external_f = ti.Vector.field(
            3, dtype=float, shape=(self.sub_steps), needs_grad=True
        )  # accumulated force from each particle
    ...
    def set_object_pose(self, d_pos, d_ori):
        ...
        self.dt_h[None] = ti.Vector(
            [
                self.init_pos[None][0] + d_pos[0],
                self.init_pos[None][1] + d_pos[1],
                self.init_pos[None][2] + d_pos[2],
            ]
        )
    ...
    def load_step_from_cache(
        self, f: ti.i32, cache_x_0: ti.types.ndarray(), cache_v_0: ti.types.ndarray()
    ):
        for p in range(self.n_particles):
    ...
    def memory_to_cache(self, t):
        ...
        self.add_step_to_cache(
            0, self.cache[cur_step_name]["x_0"], self.cache[cur_step_name]["v_0"]
        )
