...
class Contact:
    def __init__(
        self, use_tactile, use_state, dt=5e-5, total_steps=300, sub_steps=80, obj=None
    ):
        self.iter = 0
        ...
        self.decontact_ts = (
            self.press_steps
        )  # when the sensor detaches from the surface
        ...
        self.contact_idx = ti.Vector.field(
            self.num_sensor,
            dtype=int,
            shape=(self.sub_steps, self.mpm_object.n_particles),
        ...
        self.draw_pos2 = ti.Vector.field(
            2, float, self.fem_sensor1.n_verts
        )  # elastomer1's pos
    ...
    def load_pos_control(self):
        cur_pos = np.load(
            os.path.join(
                f"lr_surface_follow_{args.use_state}_tactile_{args.use_tactile}",
                "control_p_gripper_15.npy",
            )
        )
        cur_ori = np.load(
            os.path.join(
    ...
    def load_target(self):
        self.target_force[None] = ti.Vector(
            [300.0, -300.0, 0.0]
        )  # only use normal force
    ...
    def compute_force_loss(self):
        self.predict_force[None] = (
            self.fem_sensor1.inv_rot[None] @ self.contact_force[None]
        )
        self.loss[None] += (
            self.beta[None]
            * (self.predict_force[None][0] - self.target_force[None][0]) ** 2
            + (self.predict_force[None][1] - self.target_force[None][1]) ** 2
            + (self.predict_force[None][2] - self.target_force[None][2]) ** 2
        )
    ...
    def compute_loc_loss(self):
        ...
        self.loss[None] += (
            self.predict_loc[None][0] - self.target_loc[None][0]
        ) ** 2 + (self.predict_loc[None][2] - self.target_loc[None][2]) ** 2

    @ti.kernel
    def compute_sensor_dis_loss(self):
        self.loss[None] += self.alpha[None] * (
            (self.fem_sensor1.trans_h[None][0, 3] - self.target_sensor_pos[None][0])
            ** 2
            + (self.fem_sensor1.trans_h[None][1, 3] - self.target_sensor_pos[None][1])
            ** 2
            + (self.fem_sensor1.trans_h[None][2, 3] - self.target_sensor_pos[None][2])
            ** 2
        )
    ...
    def draw_external_force(self, f: ti.i32):
        ...
        for i in range(half_seg):
            f_1 = self.fem_sensor1.external_force_field[
                f, self.fem_sensor1.contact_seg[i][0]
            ]
...
def main():
    ...
    for opts in range(num_opt_steps):
        ...
        if not os.path.exists(
            f"lr_surface_follow_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}"
        ):
            os.mkdir(
                f"lr_surface_follow_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}"
            )
        ...
        if opts % 5 == 0 or opts == num_opt_steps - 1:
            ...
            plt.savefig(
                os.path.join(
                    f"lr_surface_follow_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}",
                    f"surface_follow_{args.use_state}_tactile_{args.use_tactile}_{opts}.png",
                )
            )
            np.save(
                os.path.join(
