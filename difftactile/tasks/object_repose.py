...
class Contact:
    def __init__(
        ...
        self.predict_force1 = ti.Vector.field(
            self.dim, float, (self.total_steps), needs_grad=True
        )
        ...
        self.draw_grid_3d = ti.Vector.field(
            3, dtype=float, shape=(self.mpm_object.n_grid**3)
        )
        self.color_grid_3d = ti.Vector.field(
            3, dtype=float, shape=(self.mpm_object.n_grid**3)
        )
    ...
    def compute_force_loss(self, t: ti.i32):
        self.predict_force1[t] = (
            self.fem_sensor1.inv_rot[None] @ self.contact_force1[None]
        )

        self.loss[None] += self.beta[None] * (
            (self.predict_force1[t][1] - self.target_force1[t][1]) ** 2
            + (self.predict_force1[t][0] - self.target_force1[t][0]) ** 2
        )
...
def main():
    ...
    for opts in range(0, num_opt_steps):
        ...
        if not os.path.exists(
            f"lr_object_repose_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}"
        ):
            os.mkdir(
                f"lr_object_repose_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}"
            )
        ...
        if opts % 5 == 0 or opts == num_opt_steps - 1:
            ...
            plt.savefig(
                os.path.join(
                    f"lr_object_repose_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}",
                    f"object_repose_state_{args.use_state}_tactile_{args.use_tactile}_{opts}.png",
                )
            )
            np.save(
                os.path.join(
