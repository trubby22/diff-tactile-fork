...
class Contact:
    ...
    def collision(self, f:ti.i32):
        for i, j, k in ti.ndrange(self.mpm_object.n_grid, self.mpm_object.n_grid, self.mpm_object.n_grid):
            if self.mpm_object.grid_occupy[f, i, j, k] == 1:
                ...
                if contact_flag1:
                    ext_v1, _, _ = self.calculate_contact_force(cur_sdf1, -1*cur_norm_v1, -1*cur_relative_v1)
    ...
    def draw_markers(self, init_markers, cur_markers, gui):
        ...
        scale = img_width
        rescale = 1.8
        draw_points = rescale * (init_markers - [int(img_width / 2.0), int(img_height / 2.0)]) / scale + [0.5, 0.5]
