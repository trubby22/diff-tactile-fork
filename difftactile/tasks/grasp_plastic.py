...
class Contact:
    ...
    def compute_pos_loss(self):
        self.loss[None] += self.gamma[None] * (
...
def main():
    ...
    for opts in range(opt_steps):
        ...
        for ts in range(total_steps - 1):
            ...
            if (
                opts == 0
                and abs(contact_model.predict_force1[None][0]) > THRE
                and abs(contact_model.predict_force2[None][0]) > THRE
                and reset_flag == False
            ):  # 1e6 is nice
                reset_flag = True
        ...
        if not os.path.exists(
            f"grasp_plastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}"
        ):
            os.mkdir(
                f"grasp_plastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}"
            )
        ...
        if opts % 5 == 0 or opts == opt_steps - 1:
            ...
            plt.savefig(
                os.path.join(
                    f"grasp_plastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}",
                    f"grasp_plastic_{args.use_state}_tactile_{args.use_tactile}_{opts}.png",
                )
            )
            np.save(
                os.path.join(
