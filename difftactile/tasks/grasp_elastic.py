...
class Contact:
    ...
    def compute_pos_loss(self):
        self.loss[None] += self.alpha[None]*( (self.predict_obj_pos[None][0] - self.target_obj_pos[None][0])**2 + (self.predict_obj_pos[None][1] - self.target_obj_pos[None][1])**2 + (self.predict_obj_pos[None][2] - self.target_obj_pos[None][2])**2)
...
def main():
    ...
    for opts in range(opt_steps):
        ...
        for ts in range(total_steps-1):
            ...
            print("slip", contact_model.gripper.fem_sensor1.trans_h[None][1,3] - contact_model.predict_obj_pos[None][1])
        ...
        if not os.path.exists(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}"):
                os.mkdir(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}")
        ...
        if opts %5 == 0 or opts == opt_steps-1:
            ...
            plt.plot(losses)
            plt.savefig(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}",f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{opts}.png"))
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}", f"control_p_gripper_{opts}.npy"), contact_model.p_gripper.to_numpy())
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}", f"control_o_gripper_{opts}.npy"), contact_model.o_gripper.to_numpy())
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}", f"control_w_gripper_{opts}.npy"), contact_model.w_gripper.to_numpy())
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}", f"losses_{opts}.npy"), np.array(losses))
        ...
        if loss_frame <= np.min(losses):
            ...
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}","control_p_gripper_best.npy"), best_p)
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}","control_o_gripper_best.npy"), best_o)
            np.save(os.path.join(f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}","control_w_gripper_best.npy"), best_w)
