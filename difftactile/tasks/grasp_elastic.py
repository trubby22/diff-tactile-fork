...
class Contact:
    def __init__(self, dt=5e-5, total_steps=300, substeps=50, obj=None):
        ...
        self.fem_sensor_pos1 = ti.Vector.field(
            3, float, shape=(self.gripper.fem_sensor1.n_verts), needs_grad=True
        )
        self.fem_sensor_pos2 = ti.Vector.field(
            3, float, shape=(self.gripper.fem_sensor2.n_verts), needs_grad=True
        )
        ...
        self.marker_offset_norm1 = ti.Vector.field(
            2, float, shape=(), needs_grad=True
        )  # To get the contact detect flag
        self.marker_offset1 = ti.Vector.field(2, float, shape=(), needs_grad=True)

        self.marker_offset_norm2 = ti.Vector.field(
            2, float, shape=(), needs_grad=True
        )  # To get the contact detect flag
        ...
        self.predict_obj_pos = ti.Vector.field(
            self.dim, float, shape=(), needs_grad=True
        )

        # Target grid
        self.target_grid = ti.field(
            dtype=float,
            shape=(
                self.mpm_object.n_grid,
                self.mpm_object.n_grid,
    ...
    def compute_force_loss(self):
        ...
        if (
            abs(self.predict_force1[None][0]) > 1e-3
            and abs(self.predict_force2[None][0]) > 1e-3
        ):
            self.loss[None] += self.beta[None] * (
                abs(self.predict_force1[None][0])
                / abs(self.friction_coeff[None] * self.predict_force1[None][1])
                + abs(self.predict_force2[None][0])
                / abs(self.friction_coeff[None] * self.predict_force2[None][1])
            )
        else:
            self.loss[None] += 0
    ...
    def compute_marker_motion(self):
        for p in range(self.gripper.fem_sensor1.num_markers):
            self.marker_offset1[None] += (
                1
                / self.gripper.fem_sensor1.num_markers
                * (
                    self.gripper.fem_sensor1.predict_markers[p]
                    - self.gripper.fem_sensor1.virtual_markers[p]
                )
            )
            self.marker_offset2[None] += (
                1
                / self.gripper.fem_sensor2.num_markers
                * (
                    self.gripper.fem_sensor2.predict_markers[p]
                    - self.gripper.fem_sensor2.virtual_markers[p]
                )
            )
            self.marker_offset_norm1[None] += (
                1
                / self.gripper.fem_sensor1.num_markers
                * abs(
                    self.gripper.fem_sensor1.predict_markers[p]
    ...
    def compute_marker_loss(self):
        self.loss[None] += self.beta[None] * (
            (self.marker_offset1[None][0]) ** 2
            + (self.marker_offset1[None][1]) ** 2
            + (self.marker_offset2[None][0]) ** 2
            + (self.marker_offset2[None][1]) ** 2
        )
    ...
    def compute_obj_pos(self, f: ti.i32):
        for p in range(self.mpm_object.n_particles):
            self.predict_obj_pos[None] += (
                1 / self.mpm_object.n_particles
            ) * self.mpm_object.x_0[f, p]

    @ti.kernel
    def compute_pos_loss(self):
        self.loss[None] += self.alpha[None] * (
            (self.predict_obj_pos[None][0] - self.target_obj_pos[None][0]) ** 2
            + (self.predict_obj_pos[None][1] - self.target_obj_pos[None][1]) ** 2
            + (self.predict_obj_pos[None][2] - self.target_obj_pos[None][2]) ** 2
        )
        ...
        for i, j, k in ti.ndrange(
            ...
            if self.mpm_object.grid_occupy[f, i, j, k] == 1:
                ...
                cur_sdf2, cur_norm_v2, cur_relative_v2, contact_flag2 = (
                    self.gripper.fem_sensor2.find_sdf(cur_p, cur_v, min_idx2, f)
                )
                cur_sdf1, cur_norm_v1, cur_relative_v1, contact_flag1 = (
                    self.gripper.fem_sensor1.find_sdf(cur_p, cur_v, min_idx1, f)
                )
                if contact_flag1:
                    ...
                    self.gripper.fem_sensor1.update_contact_force(
                        min_idx1, -1 * ext_v1, f
                    )
                ...
                if contact_flag2:
                    ...
                    self.gripper.fem_sensor2.update_contact_force(
                        min_idx2, -1 * ext_v2, f
                    )
    ...
    def add_virtual_force(self, f: ti.i32):
        for i, j, k in ti.ndrange(
            ...
            if self.mpm_object.grid_occupy[f, i, j, k] == 1:
                self.mpm_object.grid_f[f, i, j, k] = (
                    ti.Vector([0.0, 0.0, 0.0]) * self.dt
                )
    ...
    def load_target(self):
        self.target_obj_pos[None] = ti.Vector([5.5, 2.5, 5.8])  # only use normal force
    ...
    def clear_loss_grad(self):
        ...
        self.predict_obj_pos.grad[None].fill(0.0)
    ...
    def init_control_parameters(self):
        ...
        ry = 0.0
        rz = 0.0
        ws = 5.0  # ws > 0, gripper width will get shorter

        for i in range(0, half_traj):
        ...
        for i in range(half_traj, self.total_steps):
            self.p_gripper[i] = ti.Vector([vx, vy, vz])
            self.o_gripper[i] = ti.Vector([rx, ry, rz])
            self.w_gripper[i] = ws
...
def main():
    ...
    contact_model = Contact(
        total_steps=total_steps, substeps=sub_steps, dt=dt, obj=obj_name
    )
    reset = False
    cnt = 0
    ...
    for opts in range(opt_steps):
        ...
        for ts in range(total_steps - 1):
            ...
            contact_model.compute_pos_loss()
            ...
            print(
                ...
                f_shear.norm(),
                "norm force",
                contact_model.predict_force1[None][1],
                "mu multiple norm force",
                contact_model.friction_coeff[None]
                * contact_model.predict_force1[None][1],
            )

            print(
                "slip",
                contact_model.gripper.fem_sensor1.trans_h[None][1, 3]
                - contact_model.predict_obj_pos[None][1],
            )
            ...
            if not Off_screen:
                ...
                scene.particles(
                    contact_model.draw_pos, color=(0.68, 0.2 == 6, 0.19), radius=0.02
                )
                scene.mesh(
                    contact_model.gripper.gripper_base.ti_vertices,
                    contact_model.gripper.gripper_base.ti_faces,
                    contact_model.gripper.gripper_base.ti_normals,
                )
                scene.mesh(
                    contact_model.gripper.gripper_finger1.ti_vertices,
                    contact_model.gripper.gripper_finger1.ti_faces,
                    contact_model.gripper.gripper_finger1.ti_normals,
                )
                scene.mesh(
                    contact_model.gripper.gripper_finger2.ti_vertices,
                    contact_model.gripper.gripper_finger2.ti_faces,
                    contact_model.gripper.gripper_finger2.ti_normals,
                )
                scene.particles(
                    contact_model.fem_sensor_pos1, color=(0.1, 0.5, 0.5), radius=0.05
                )
                scene.particles(
                    contact_model.fem_sensor_pos2, color=(0.1, 0.5, 0.5), radius=0.05
                )
                canvas.scene(scene)
                window.show()
        ...
        if USE_STATE:
            ...
            contact_model.compute_pos_loss()
            print("loss", contact_model.loss[None] - form_loss)
            contact_model.compute_pos_loss.grad()
            contact_model.compute_obj_pos.grad(sub_steps - 1)
        ...
        if not os.path.exists(
            f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}"
        ):
            os.mkdir(
                f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}"
            )
        ...
        if opts % 5 == 0 or opts == opt_steps - 1:
            ...
            plt.savefig(
                os.path.join(
                    f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{args.obj_name}",
                    f"grasp_elastic_{args.use_state}_tactile_{args.use_tactile}_{opts}.png",
                )
            )
            np.save(
                os.path.join(
