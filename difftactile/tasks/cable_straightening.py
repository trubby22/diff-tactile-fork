...
class Contact:
    def __init__(
        self, use_state, use_tactile, dt=5e-5, total_steps=300, sub_steps=50, obj=None
    ):
        self.dt = dt
        ...
        self.gripper = Gripper(
            data_path, sub_steps=sub_steps, mesh_vis_scale=self.view_scale
        )
        ...
        self.contact_idx = ti.Vector.field(
            self.num_sensor,
            dtype=int,
            shape=(self.sub_steps, self.rope_object.n_particles),
        )  # indicating the contact seg idx of fem sensor for closested triangle mesh
        self.contact_flag = ti.field(
            dtype=int, shape=(self.sub_steps, self.rope_object.n_particles)
        )
        self.total_ext_f = ti.Vector.field(3, dtype=float, shape=())

        # Transloation, Rotation and Width speed of the gripper
        self.p_gripper = ti.Vector.field(
            self.dim, dtype=ti.f32, shape=(self.total_steps), needs_grad=True
        )
        self.o_gripper = ti.Vector.field(
            self.dim, dtype=ti.f32, shape=(self.total_steps), needs_grad=True
        )
        self.w_gripper = ti.field(
            dtype=ti.f32, shape=(self.total_steps), needs_grad=True
        )
        ...
        self.predict_force1 = ti.Vector.field(
            self.dim, float, shape=(), needs_grad=True
        )
        self.predict_force2 = ti.Vector.field(
            self.dim, float, shape=(), needs_grad=True
        )
        self.contact_force1 = ti.Vector.field(
            self.dim, float, shape=(), needs_grad=True
        )
        self.contact_force2 = ti.Vector.field(
            self.dim, float, shape=(), needs_grad=True
        )
        ...
        ## 2D visualization
        self.surf_offset1 = ti.Vector.field(
            2, float, self.gripper.fem_sensor1.num_triangles
        )
        self.surf_init_pos1 = ti.Vector.field(
            2, float, self.gripper.fem_sensor1.num_triangles
        )
        self.press_offset1 = ti.field(float, self.gripper.fem_sensor1.num_triangles)

        self.draw_pos1 = ti.Vector.field(
            2, float, self.gripper.fem_sensor1.n_verts
        )  # elastomer1's pos
        self.draw_pos2 = ti.Vector.field(
            2, float, self.gripper.fem_sensor2.n_verts
        )  # elastomer2's pos
        self.draw_pos = ti.Vector.field(
            2, dtype=float, shape=(self.rope_object.n_particles)
        )  # rope's pos
        self.draw_tableline = ti.Vector.field(3, dtype=float, shape=(2 * 4))  # table

        # 3d viz
        self.draw_pos_3d = ti.Vector.field(
            3, dtype=float, shape=(self.rope_object.n_particles)
        )
        self.draw_fem1_3d = ti.Vector.field(
            3, dtype=float, shape=(self.gripper.fem_sensor1.n_verts)
        )
        self.draw_fem2_3d = ti.Vector.field(
            3, dtype=float, shape=(self.gripper.fem_sensor2.n_verts)
        )
        self.color_fem1_3d = ti.Vector.field(
            3, dtype=float, shape=(self.gripper.fem_sensor1.n_verts)
        )
        self.color_fem2_3d = ti.Vector.field(
            3, dtype=float, shape=(self.gripper.fem_sensor2.n_verts)
        )
    ...
    def clamp_grid(self, f: ti.i32):
        for p in range(self.rope_object.n_particles):
            self.rope_object.pos.grad[f, p] = ti.math.clamp(
                self.rope_object.pos.grad[f, p], -1000.0, 1000.0
            )
            self.rope_object.vel.grad[f, p] = ti.math.clamp(
                self.rope_object.vel.grad[f, p], -1000.0, 1000.0
            )

        for i in range(self.gripper.fem_sensor1.n_verts):
            self.gripper.fem_sensor1.pos.grad[f, i] = ti.math.clamp(
                self.gripper.fem_sensor1.pos.grad[f, i], -1000.0, 1000.0
            )
            self.gripper.fem_sensor1.vel.grad[f, i] = ti.math.clamp(
                self.gripper.fem_sensor1.vel.grad[f, i], -1000.0, 1000.0
            )

        for i in range(self.gripper.fem_sensor2.n_verts):
            self.gripper.fem_sensor2.pos.grad[f, i] = ti.math.clamp(
                self.gripper.fem_sensor2.pos.grad[f, i], -1000.0, 1000.0
            )
            self.gripper.fem_sensor2.vel.grad[f, i] = ti.math.clamp(
                self.gripper.fem_sensor2.vel.grad[f, i], -1000.0, 1000.0
            )
        ...
        if shear_vel_norm_p0 > 1e-4:
            shear_factor_p0 = (
                ...
                * ti.min(
                    ...
                    self.friction_coeff[None] * normal_factor_p0.norm(),
                )
            )
    ...
    def compute_contact_force(self, f: ti.i32):
        for i in range(self.gripper.fem_sensor1.num_triangles):
            ...
            self.contact_force1[None] += (
                1 / 6 * self.gripper.fem_sensor1.external_force_field[f, a]
            )
            self.contact_force1[None] += (
                1 / 6 * self.gripper.fem_sensor1.external_force_field[f, b]
            )
            self.contact_force1[None] += (
                1 / 6 * self.gripper.fem_sensor1.external_force_field[f, c]
            )
        for i in range(self.gripper.fem_sensor2.num_triangles):
            a, b, c = self.gripper.fem_sensor2.contact_seg[i]
            self.contact_force2[None] += (
                1 / 6 * self.gripper.fem_sensor2.external_force_field[f, a]
            )
            self.contact_force2[None] += (
                1 / 6 * self.gripper.fem_sensor2.external_force_field[f, b]
            )
            self.contact_force2[None] += (
                1 / 6 * self.gripper.fem_sensor2.external_force_field[f, c]
            )
    ...
    def compute_contact_loc(self, f: ti.i32):
        for i in range(self.gripper.fem_sensor1.num_surface):
            ...
            inv_pos = self.gripper.fem_sensor1.inv_trans_h[None] @ ti.Vector(
                [cur_pos[0], cur_pos[1], cur_pos[2], 1.0]
            )
        ...
        for i in range(self.gripper.fem_sensor2.num_surface):
            ...
            inv_pos = self.gripper.fem_sensor2.inv_trans_h[None] @ ti.Vector(
                [cur_pos[0], cur_pos[1], cur_pos[2], 1.0]
            )
            pos = ti.Vector([inv_pos[0], inv_pos[1], inv_pos[2]])
    ...
    def compute_force_loss(self):
        self.predict_force1[None] = (
            self.gripper.fem_sensor1.inv_rot[None] @ self.contact_force1[None]
        )
        self.predict_force2[None] = (
            self.gripper.fem_sensor2.inv_rot[None] @ self.contact_force2[None]
        )
        ...
        self.loss[None] += (
            ...
            * self.wf
            * (
                (self.predict_force2[None][1] - self.target_force2[None][1]) ** 2
                + (self.predict_force2[None][0] - self.target_force2[None][0]) ** 2
            )
        )
    ...
    def compute_loc_loss(self):
        self.loss[None] += (
            ...
            * self.wl
            * (
                (self.target_loc1[None][0] - self.predict_loc1[None][0]) ** 2
                + (self.target_loc1[None][2] - self.predict_loc1[None][2]) ** 2
            )
        )
        self.loss[None] += (
            self.beta
            * self.wl
            * (
                (self.target_loc2[None][0] - self.predict_loc2[None][0]) ** 2
                + (self.target_loc2[None][2] - self.predict_loc2[None][2]) ** 2
            )
        )
    ...
    def compute_rope_pos_loss(self, f: ti.i32):
        for p in range(self.rope_object.n_particles):
            self.loss[None] += (
                self.alpha
                * (self.rope_object.pos[f, p][0] - self.rope_object.init_pos[p][0]) ** 2
                + (self.rope_object.pos[f, p][1] - self.rope_object.init_pos[p][1]) ** 2
    ...
    def collision(self, f: ti.i32):
        for p in range(self.rope_object.n_particles):
            ...
            cur_sdf1, cur_norm_v1, cur_relative_v1, contact_flag1 = (
                self.gripper.fem_sensor1.find_sdf(
                    cur_p, cur_v, min_idx1, f, self.rope_object.p_rad
                )
            )
            cur_sdf2, cur_norm_v2, cur_relative_v2, contact_flag2 = (
                self.gripper.fem_sensor2.find_sdf(
                    cur_p, cur_v, min_idx2, f, self.rope_object.p_rad
                )
            )
            if contact_flag1:
                ext_v1, ext_n1, ext_t1 = self.calculate_contact_force(
            ...
            if contact_flag2:
                ext_v2, ext_n2, ext_t2 = self.calculate_contact_force(
                    cur_sdf2, -1 * cur_norm_v2, -1 * cur_relative_v2
                )
            ...
            if contact_flag1 or contact_flag2:
                ...
                self.rope_object.travel_info[f + 1, p] = self.rope_object.travel_info[
                    f, p
                ]
    ...
    def draw_markers_cv(self, tracked_markers, init_markers, showScale):
        ...
        for i in range(init_markers.shape[0]):
            ...
            cv2.arrowedLine(
                ...
                (
                    int(init_markers[i, 0] + marker_motion[0] * showScale),
                    int(init_markers[i, 1] + marker_motion[1] * showScale),
                ),
                (0, 255, 255),
                2,
            )
    ...
    def load_target(self):
        ...
        self.target_force1[None] = ti.Vector(
            [-300.0, -300.0, 0.0]
        )  # maybe don't penalize fx
    ...
    def draw_external_force(self, f: ti.i32):
        ...
        for i in range(half_seg):
            f_1 = self.gripper.fem_sensor1.external_force_field[
                f, self.gripper.fem_sensor1.contact_seg[i][0]
            ]
            i_1 = self.gripper.fem_sensor1.virtual_pos[
                f, self.gripper.fem_sensor1.contact_seg[i][0]
            ]
    ...
    def draw_surface(self, f: ti.i32):
        ...
        half_seg = self.gripper.fem_sensor1.num_triangles  # //2
        for i in range(half_seg):
            p_1 = self.gripper.fem_sensor1.pos[
                f, self.gripper.fem_sensor1.contact_seg[i][0]
            ]  # triangle's 1st node
            # b_1 = self.fem_sensor1.pos[f, self.fem_sensor1.base_seg[i][0]] # triangle's 1st node
            i_1 = self.gripper.fem_sensor1.init_x[
                self.gripper.fem_sensor1.contact_seg[i][0]
            ]
    ...
    def draw_perspective(self, f: ti.i32):
        ...
        for i in range(self.gripper.fem_sensor1.n_verts):
            x, y, z = (
                self.gripper.fem_sensor1.pos[f, i][0] - offset,
                self.gripper.fem_sensor1.pos[f, i][1] - offset,
                self.gripper.fem_sensor1.pos[f, i][2] - offset,
            )
        ...
        for i in range(self.gripper.fem_sensor2.n_verts):
            x, y, z = (
                self.gripper.fem_sensor2.pos[f, i][0] - offset,
                self.gripper.fem_sensor2.pos[f, i][1] - offset,
                self.gripper.fem_sensor2.pos[f, i][2] - offset,
            )
            xx, zz = x * c_p + z * s_p, z * c_p - x * s_p
            u, v = xx, y * c_t + zz * s_t
        ...
        for i in range(self.rope_object.n_particles):
            x, y, z = (
                self.rope_object.pos[f, i][0] - offset,
                self.rope_object.pos[f, i][1] - offset,
                self.rope_object.pos[f, i][2] - offset,
            )
            xx, zz = x * c_p + z * s_p, z * c_p - x * s_p
            u, v = xx, y * c_t + zz * s_t
    ...
    def draw_3d_scene(self, f: ti.i32):
        ...
        for p in range(self.gripper.fem_sensor1.num_surface):
            ...
            self.draw_fem1_3d[p] = (
                self.gripper.fem_sensor1.pos[f, idx] / self.view_scale
            )
            self.color_fem1_3d[p] = ti.Vector([0.9, 0.7, 0.8]) + 2.0 * (
                self.gripper.fem_sensor1.pos[f, idx]
                - self.gripper.fem_sensor1.virtual_pos[f, idx]
            )

        for p in range(self.gripper.fem_sensor2.num_surface):
            idx = self.gripper.fem_sensor2.surface_id[p]
            self.draw_fem2_3d[p] = (
                self.gripper.fem_sensor2.pos[f, idx] / self.view_scale
            )
            self.color_fem2_3d[p] = ti.Vector([0.02, 0.8, 0.9]) + 2.0 * (
                self.gripper.fem_sensor2.pos[f, idx]
                - self.gripper.fem_sensor2.virtual_pos[f, idx]
            )
...
def main():
    ...
    for opts in range(num_opt_steps):
        ...
        for ts in range(num_total_steps - 1):
            ...
            print(
                "Predict force 1/2",
                contact_model.predict_force1[None],
                contact_model.predict_force2[None],
            )
            ...
            print(
                "Predict loc 1/2",
                contact_model.predict_loc1[None],
                contact_model.predict_loc2[None],
            )
            ...
            if not Off_screen:
                ...
                gui1.circles(
                    viz_scale * contact_model.draw_pos.to_numpy() + viz_offset,
                    radius=15,
                    color=0x039DFC,
                )
                gui1.circles(
                    viz_scale * contact_model.draw_pos1.to_numpy() + viz_offset,
                    radius=2,
                    color=0xE6C949,
                )
                gui1.circles(
                ...
                contact_model.draw_triangles(
                    contact_model.gripper.fem_sensor1,
                    gui3,
                    0,
                    0,
                    90,
                ...
                scene.particles(
                    ...
                    radius=contact_model.p_rad / contact_model.view_scale,
                )
                scene.particles(
                ...
                scene.particles(
                    contact_model.draw_fem2_3d,
                    per_vertex_color=contact_model.color_fem2_3d,
                    radius=0.01,
                )
                scene.mesh(
                    contact_model.gripper.gripper_base.vis_ti_vertices,
                    contact_model.gripper.gripper_base.vis_ti_faces,
                    contact_model.gripper.gripper_base.vis_ti_normals,
                )
                scene.mesh(
                    contact_model.gripper.gripper_finger1.vis_ti_vertices,
                    contact_model.gripper.gripper_finger1.vis_ti_faces,
                    contact_model.gripper.gripper_finger1.vis_ti_normals,
                )
                scene.mesh(
                    contact_model.gripper.gripper_finger2.vis_ti_vertices,
                    contact_model.gripper.gripper_finger2.vis_ti_faces,
                    contact_model.gripper.gripper_finger2.vis_ti_normals,
                )
                scene.lines(
        ...
        for ts in range(num_total_steps - 2, -1, -1):
            ...
            print(
                "P/O updated: ",
                contact_model.p_gripper[ts],
                contact_model.o_gripper[ts],
                contact_model.w_gripper[ts],
            )
        ...
        if not os.path.exists(
            f"lr_cable_manipulation_{args.use_state}_tactile_{args.use_tactile}"
        ):
            os.mkdir(
                f"lr_cable_manipulation_{args.use_state}_tactile_{args.use_tactile}"
            )
        ...
        if opts % 5 == 0 or opts == num_opt_steps - 1:
            ...
            plt.savefig(
                os.path.join(
                    f"lr_cable_manipulation_{args.use_state}_tactile_{args.use_tactile}",
                    f"cable_manipulation_{args.use_state}_tactile_{args.use_tactile}_{opts}.png",
                )
            )
            np.save(
                os.path.join(
                    f"lr_cable_manipulation_{args.use_state}_tactile_{args.use_tactile}",
                    f"control_p_gripper_{opts}.npy",
                ),
                contact_model.p_gripper.to_numpy(),
            )
            np.save(
                os.path.join(
                    f"lr_cable_manipulation_{args.use_state}_tactile_{args.use_tactile}",
                    f"control_o_gripper_{opts}.npy",
                ),
                contact_model.o_gripper.to_numpy(),
            )
            np.save(
                os.path.join(
                    f"lr_cable_manipulation_{args.use_state}_tactile_{args.use_tactile}",
                    f"control_w_gripper_{opts}.npy",
                ),
                contact_model.w_gripper.to_numpy(),
            )
