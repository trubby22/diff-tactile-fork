...
class FEMDomeSensor:
    def __init__(self, dt=5e-5, sub_steps=50, init_img_path=None):
        ...
        self.N_node = 320  # number of nodes in the most inner layer
        self.N_t = 2  # thickness
        self.t_res = 0.25
        self.inner_radius = 0.5  # 7.5 mm radius
        ...
        self.trans_local = ti.Matrix.field(
            4, 4, ti.f32, shape=(), needs_grad=True
        )  ## ee1 -> ee2
        ...
        self.control_vel = ti.Vector.field(
            3, float, shape=(self.n_verts), needs_grad=True
        )
    ...
    def init(self, rot_x, rot_y, rot_z, t_dx, t_dy, t_dz):
        ...
        self.inv_trans_h[None] = (
            self.trans_local[None].inverse() @ self.trans_world[None].inverse()
        )
        self.init_pos()
    ...
    @ti.kernel
    def set_trans_h(
        self, d_pos: ti.types.vector(3, float), d_ori: ti.types.vector(3, float)
    ):
    ...
    def set_pose_control(self):
        ...
        self.dtrans_h[None] = (
            self.trans_world[None] @ trans_mat @ (self.trans_world[None].inverse())
        )
    ...
    def set_control_vel(self, f: ti.i32):
        for i in range(self.n_verts):
            ...
            after_t_pos = self.dtrans_h[None] @ ti.Vector(
                [init_t_pos[0], init_t_pos[1], init_t_pos[2], 1.0]
            )  # 4 x 1 homogeneous
            self.control_vel[i][0] = (after_t_pos[0] - init_t_pos[0]) / (
                self.dt * (self.sub_steps - 1)
            )
            self.control_vel[i][1] = (after_t_pos[1] - init_t_pos[1]) / (
                self.dt * (self.sub_steps - 1)
            )
            self.control_vel[i][2] = (after_t_pos[2] - init_t_pos[2]) / (
                self.dt * (self.sub_steps - 1)
            )
    ...
    def update2(self, f: ti.i32):
        for i in range(self.n_verts):
            ...
            self.virtual_pos[f + 1, i] = (
                self.virtual_pos[f, i] + self.dt * self.control_vel[i]
            )
