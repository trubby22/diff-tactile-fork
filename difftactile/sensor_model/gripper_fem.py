...
class FEMDomeSensor:
    def __init__(self, dt=5e-5, sub_steps=50, init_img_path=None):
        ...
        self.E_init[None], self.nu_init[None] = (
            0.22e4,
            0.46,
        )  # 0.3e4, 0.445  # Young's modulus and Poisson's ratio
        ...
        self.surface_cam_loc = ti.Vector.field(
            2, float, self.num_surface, needs_grad=True
        )
        self.surface_cam_virtual_loc = ti.Vector.field(2, float, self.num_surface)

        # cam model
        self.num_k_closest = 5
        self.initial_markers, interp_idx, interp_weight = self.init_cam_model(
            init_img_path
        )
        self.num_markers = len(self.initial_markers)
        self.predict_markers = ti.Vector.field(
            2, float, self.num_markers, needs_grad=True
        )
        self.virtual_markers = ti.Vector.field(2, float, self.num_markers)

        self.interp_weight = ti.Vector.field(
            self.num_k_closest, float, self.num_markers
        )
        ...
        self.f2v = ti.Vector.field(
            4, int, self.n_cells
        )  # ids of three vertices of each face
        self.f2v.from_numpy(self.all_f2v.astype(np.int32))
        self.contact_seg = ti.Vector.field(
            3, int, self.num_triangles
        )  # surface triangle mesh
        self.contact_seg.from_numpy(self.surface_f2v.astype(np.int32))

        self.virtual_pos = ti.Vector.field(
            3, float, shape=(self.sub_steps, self.n_verts), needs_grad=True
        )
        ...
        self.external_force_field = ti.Vector.field(
            3, dtype=ti.f32, shape=(self.sub_steps, self.n_verts), needs_grad=True
        )  # contact force between FEM node to the closest particle
        ...
        self.control_vel = ti.Vector.field(
            3, float, shape=(self.sub_steps, self.n_verts), needs_grad=True
        )
    ...
    def init(self, matrix):
        self.trans_h[None] = matrix
        self.inv_trans_h[None] = self.trans_h[None].inverse()
        self.rot_h[None] = matrix[0:3, 0:3]
    ...
    def init_cam_model(self, init_img_path=None):
        ...
        cam_3D_nodes = np.array(
            [surface_nodes[:, 0], surface_nodes[:, 2], surface_nodes[:, 1]]
        ).T
    ...
    def extract_markers(self, f: ti.i32):
        for i in range(self.num_surface):
            ...
            cam_init_pos = ti.Vector(
                [inv_init_pos[0], inv_init_pos[2], inv_init_pos[1]]
            )
        ...
        for i in range(self.num_markers):
            ...
            for j in range(self.num_k_closest):
                ...
                init_loc_2d += (
                    weights[j] * self.surface_cam_virtual_loc[smallest_idx[j]]
                )
    ...
    def set_vel(self, f: ti.i32):
        for p in range(self.n_verts):
            self.vel[f, p] = self.control_vel[f, p]

    @ti.kernel
    def set_rot(self, matrix: ti.types.matrix(4, 4, ti.f32)):
        for i in range(3):
            for j in range(3):
                self.rot_h[None][i, j] = matrix[i, j]

    @ti.kernel
    def set_pos_control(self, matrix: ti.types.matrix(4, 4, ti.f32)):
        self.trans_h[None] = matrix
        self.inv_trans_h[None] = self.trans_h[None].inverse()
        self.inv_rot[None] = self.rot_h[None].inverse()

    @ti.kernel
    def set_pos_control_bp(self):
        self.inv_trans_h[None] = self.trans_h[None].inverse()
        self.inv_rot[None] = self.rot_h[None].inverse()
    ...
    def set_control_vel(
        self,
        f: ti.i32,
        finger_new: ti.types.matrix(4, 4, ti.f32),
        finger_old: ti.types.matrix(4, 4, ti.f32),
    ):
        # rot represents rotation in local coordinate

        for i in range(self.n_verts):
            # init_t_pos = self.virtual_pos[f, i]
            init_x = ti.Vector(
                [self.init_x[i][0], self.init_x[i][1], self.init_x[i][2], 1.0]
            )
            init_t_pos = finger_old @ init_x
            after_t_pos = finger_new @ init_x
            self.control_vel[f, i][0] = (after_t_pos[0] - init_t_pos[0]) / self.dt
            self.control_vel[f, i][1] = (after_t_pos[1] - init_t_pos[1]) / self.dt
            self.control_vel[f, i][2] = (after_t_pos[2] - init_t_pos[2]) / self.dt

    @ti.kernel
    def get_external_force(self, f: ti.i32):
        for k in range(self.num_triangles):
            a, b, c = self.contact_seg[k]
            self.surf_f[f] += 1 / 6 * self.external_force_field[f, a] * self.dx
            self.surf_f[f] += 1 / 6 * self.external_force_field[f, b] * self.dx
            self.surf_f[f] += 1 / 6 * self.external_force_field[f, c] * self.dx
    ...
    def init_mesh(self):
        ...
        for i in range(self.N_t):
            ...
            if i == self.N_t - 1:
                ...
                surface_f2v = (
                    num_cur_node + Delaunay(sphere_points).simplices
                )  # Mn * 3 (triangles)
    ...
    def init_pos(self):
        for idx in range(self.n_verts):
            ...
            self.pos[0, idx] = ti.Vector(
                [after_t_pos[0], after_t_pos[1], after_t_pos[2]]
            )
    ...
    def find_closest(self, grid_p, f):
        ...
        for k in range(self.num_triangles):
            ...
            offset_p = (p_c - grid_p).norm(
                self.eps
            )  # distance to the center point of the segment
    ...
    def update(self, f: ti.i32):
        for i in range(self.n_cells):
            ...
            stress = (
                self.mu[None] * (1 - 1 / (IC + 1)) * F_i
                + self.lam[None] * (J - alpha) * dJdF
            )
    ...
    def update2(self, f: ti.i32):
        for i in range(self.n_verts):
            ...
            if cond:
                v_out = self.control_vel[f, i]

            self.vel[f + 1, i] = v_out + (
                self.control_vel[f + 1, i] - self.control_vel[f, i]
            )
            self.pos[f + 1, i] = self.pos[f, i] + self.dt * v_out

            self.virtual_pos[f + 1, i] = (
                self.virtual_pos[f, i] + self.dt * self.control_vel[f, i]
            )
    ...
    def memory_to_cache(self, t):
        ...
        self.cache[cur_step_name]["pos"] = torch.zeros(
            (self.n_verts, self.dim), dtype=TC_TYPE, device=device
        )
        self.cache[cur_step_name]["vel"] = torch.zeros(
            (self.n_verts, self.dim), dtype=TC_TYPE, device=device
        )
        self.cache[cur_step_name]["trans_h"] = torch.zeros(
            (4, 4), dtype=TC_TYPE, device=device
        )
        self.cache[cur_step_name]["rot_h"] = torch.zeros(
            (3, 3), dtype=TC_TYPE, device=device
        )
        self.cache[cur_step_name]["virtual_pos"] = torch.zeros(
            (self.n_verts, self.dim), dtype=TC_TYPE, device=device
        )
        self.add_step_to_cache(
            0,
            self.cache[cur_step_name]["pos"],
            self.cache[cur_step_name]["vel"],
            self.cache[cur_step_name]["trans_h"],
            self.cache[cur_step_name]["virtual_pos"],
    ...
    def clear_step_grad(self, f: ti.i32):
        ...
        for p in range(self.n_verts):
            for t in range(f):
                ...
                self.control_vel.grad[t, p].fill(0.0)
